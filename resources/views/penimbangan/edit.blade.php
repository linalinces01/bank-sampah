...
<body>
    <h1>Form Ubah Data Penimbangan Sampah</h1>

    <div class="form-container">
        <form id="dataForm" method="POST" action="{{ route('penimbangan.update', $penimbangan->id) }}">
            @csrf
            @method('PUT')

            <label for="wasteType">Jenis Sampah:</label>
            <select id="wasteType" name="jenis_sampah" required value="{{ $penimbangan->jenis_sampah }}">
                <option value="organik">Organik</option>
                <option value="anorganik">Anorganik</option>
                <option value="sampah_campuran">Sampah Campuran</option>
            </select>
            <label for="weight">Berat Sampah (kg):</label>
            <input type="number" id="weight" name="berat_sampah" required value="{{ $penimbangan->berat_sampah }}">
